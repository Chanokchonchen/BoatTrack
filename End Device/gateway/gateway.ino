 ...
 * 
 */
PT_THREAD(taskAdvertise(struct pt* pt)) {
  ...
  PT_BEGIN(pt);
  for (;;) {
    // stop advertising when there was a recent report reception
    // TODO convert 5000 to a configurable parameter
    PT_WAIT_UNTIL(pt, millis() - last_report_rx_time > 5000);
    PT_WAIT_RADIO_READY(pt,ts);
    DEBUG_PRINTF("TX: ADV seq=%d\r\n",seq);
    PacketAdvertise adv;
    adv.seq = seq++;
    radio.send(0xFF,(uint8_t*)&adv,sizeof(adv));
    ts = millis();
    PT_WAIT_RADIO_READY(pt,ts);
    PT_DELAY(pt,config.advertise_interval*1000,ts);
  }
  PT_END(pt);
}
